...
const tests = [
  ...
  },
  {
    ...
    test: async () => {
      ...
      console.log(`  Repo: ${repo.full_name} (猸?${repo.stargazers_count})`);
      return true;
    }
  },
  {
...
async function runTests() {
  console.log('馃И OpenClaw GitHub Skill - Test Suite\n');
  ...
  for (const { name, test } of tests) {
    ...
    try {
      ...
      console.log(`鉂?FAIL: ${error.message}`);
      failed++;
    }
  }
  
  console.log('\n' + '=' .repeat(50));
  console.log(`\n馃搳 Results: ${passed} passed, ${failed} failed\n`);
  
  if (failed > 0) {
    console.log('鉂?Some tests failed. Check your configuration.');
    process.exit(1);
  } else {
    console.log('鉁?All tests passed! Your GitHub skill is working correctly.');
    process.exit(0);
  }
}
