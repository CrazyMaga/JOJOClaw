...
const TEMPLATES = {
  'session-state': `# SESSION-STATE.md 鈥?Active Working Memory
...
`,
  ...
  'memory-md': `# MEMORY.md 鈥?Long-Term Memory
...
---
*Curated memory 鈥?distill insights from daily logs here*
`,

  'daily-template': `# {{DATE}} 鈥?Daily Log
...
const commands = {
  init: () => {
    console.log('馃 Initializing Elite Longterm Memory...\n');
    
    // Create SESSION-STATE.md
    if (!fs.existsSync('SESSION-STATE.md')) {
      fs.writeFileSync('SESSION-STATE.md', TEMPLATES['session-state']);
      console.log('鉁?Created SESSION-STATE.md (Hot RAM)');
    } else {
      console.log('鈥?SESSION-STATE.md already exists');
    }
    
    // Create MEMORY.md
    if (!fs.existsSync('MEMORY.md')) {
      fs.writeFileSync('MEMORY.md', TEMPLATES['memory-md']);
      console.log('鉁?Created MEMORY.md (Curated Archive)');
    } else {
      console.log('鈥?MEMORY.md already exists');
    }
    
    // Create memory directory
    if (!fs.existsSync('memory')) {
      fs.mkdirSync('memory', { recursive: true });
      console.log('鉁?Created memory/ directory');
    } else {
      console.log('鈥?memory/ directory already exists');
    }
    ...
    if (!fs.existsSync(todayFile)) {
      ...
      console.log(`鉁?Created ${todayFile}`);
    }
    
    console.log('\n馃帀 Elite Longterm Memory initialized!');
    console.log('\nNext steps:');
  ...
  today: () => {
    ...
    if (!fs.existsSync(todayFile)) {
      ...
      console.log(`鈥?${todayFile} already exists`);
    }
  },
  
  status: () => {
    console.log('馃 Elite Longterm Memory Status\n');
    
    // Check SESSION-STATE.md
    if (fs.existsSync('SESSION-STATE.md')) {
      const stat = fs.statSync('SESSION-STATE.md');
      console.log(`鉁?SESSION-STATE.md (${(stat.size / 1024).toFixed(1)}KB, modified ${stat.mtime.toLocaleString()})`);
    } else {
      console.log('鉁?SESSION-STATE.md missing');
    }
    ...
    if (fs.existsSync('MEMORY.md')) {
      ...
      console.log(`鉁?MEMORY.md (${lines} lines, ${(stat.size / 1024).toFixed(1)}KB)`);
    } else {
      console.log('鉁?MEMORY.md missing');
    }
    
    // Check memory directory
    if (fs.existsSync('memory')) {
      const files = fs.readdirSync('memory').filter(f => f.endsWith('.md'));
      console.log(`鉁?memory/ (${files.length} daily logs)`);
    } else {
      console.log('鉁?memory/ directory missing');
    }
    
    // Check LanceDB
    const lancedbPath = path.join(process.env.HOME, '.clawdbot/memory/lancedb');
    if (fs.existsSync(lancedbPath)) {
      console.log('鉁?LanceDB vectors initialized');
    } else {
      console.log('鈥?LanceDB not initialized (optional)');
    }
  },
  
  help: () => {
    console.log(`
馃 Elite Longterm Memory CLI
