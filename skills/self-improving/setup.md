...
```markdown
- **Self-improving:** `~/self-improving/` (via `self-improving` skill) 鈥?execution-improvement memory (preferences, workflows, style patterns, what improved/worsened outcomes)
```
...
```markdown
- When someone says "remember this" 鈫?if it's factual context/event, update `memory/YYYY-MM-DD.md`; if it's a correction, preference, workflow/style choice, or performance lesson, log it in `~/self-improving/`
- Explicit user correction 鈫?append to `~/self-improving/corrections.md` immediately
- Reusable global rule or preference 鈫?append to `~/self-improving/memory.md`
- Domain-specific lesson 鈫?append to `~/self-improving/domains/<domain>.md`
- Project-only override 鈫?append to `~/self-improving/projects/<project>.md`
...
馃敟 HOT (always loaded):
   memory.md: 0 entries

馃尅锔?WARM (load on demand):
   projects/: 0 files
   domains/: 0 files
...
- [ ] Check memory.md line count (should be 鈮?00)
