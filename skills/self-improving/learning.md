...
Global: applies everywhere
  鈹斺攢鈹€ Domain: applies to category (code, writing, comms)
       鈹斺攢鈹€ Project: applies to specific context
            鈹斺攢鈹€ Temporary: applies to this session only
```
...
Agent: 鈫?Moves to Confirmed Preferences
       鈫?Removes from correction counter
       鈫?Cites source on future use
```

## Pattern Evolution

### Stages
1. **Tentative** 鈥?Single correction, watch for repetition
2. **Emerging** 鈥?2 corrections, likely pattern
3. **Pending** 鈥?3 corrections, ask for confirmation
4. **Confirmed** 鈥?User approved, permanent unless reversed
5. **Archived** 鈥?Unused 90+ days, preserved but inactive
